(function () {
  const layer: HTMLInputElement = document.querySelector(".input-layer")!;
  ...
  function drawTree() {
    ...
    for (let i = 1; i <= layerValue; i++) {
      result!.innerHTML += `<pre class="green" >${blank.repeat(
        layerValue - i
      )}${tree.repeat(2 * i - 1)}</pre>`;
    }

    result!.innerHTML += `<pre class="green" >${blank.repeat(
      layerValue - 1
    )}${tree}</pre><pre class="green" >${blank.repeat(
      layerValue - 1
    )}${tree}</pre>`;
  }
})();
